...
import {
  ...
  FileTextOutlined, ClockCircleOutlined, RobotOutlined,
...
export default function AiLabelSchoolDashboard() {
  ...
  }, [city, nameFilter, districtFilter, stageFilter, statusFilter, trialProductFilter, productFilter, cooperationProductFilter, keyPersonFilter, streetFilter]);
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `${city?.name || '城市'}_AI标签校名单_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  const statCards = [
    { title: 'AI标签学校', value: aiLabelData.total, icon: <RobotOutlined />, color: '#1677ff' },
    { title: '已合作', value: aiLabelData.cooperating, icon: <CheckCircleOutlined />, color: '#10b981' },
    { title: '试用中', value: aiLabelData.trialing, icon: <ExperimentOutlined />, color: '#f59e0b' },
    { title: '仅汇报', value: aiLabelData.reported, icon: <FileTextOutlined />, color: '#8b5cf6' },
    { title: '待开发', value: aiLabelData.pending, icon: <ClockCircleOutlined />, color: '#94a3b8' },
  ];

  const districtNames = useMemo(() => {
    if (!city) return [];
    return [...new Set(city.districts.map((d) => d.name))];
  }, [city]);

  const stageNames = useMemo(() => {
    return [...new Set(aiLabelData.schools.map((s) => s.stage).filter(Boolean))] as string[];
  }, [aiLabelData.schools]);
  ...
  return (
    <div>
      ...
      <SchoolAnalytics schools={aiLabelData.schools} groupBy="district" groupLabel="区县" />

      <Card title={<Space><RobotOutlined style={{ color: '#1677ff' }} /><span style={{ fontWeight: 600, color: '#1e293b' }}>人工智能标签校明细</span></Space>}
        ...
        <Table rowKey="id" columns={columns} dataSource={aiLabelData.schools} size="middle" scroll={{ x: 1000 }}
