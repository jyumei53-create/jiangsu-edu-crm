...
import {
  ...
  FileTextOutlined, ClockCircleOutlined, EditOutlined, EnvironmentOutlined,
...
export default function EssayProjectDashboard() {
  ...
  const essayData = useMemo(() => {
    ...
    for (const d of city.districts) {
      for (const s of d.schools) {
        if (s.products && s.products.includes('作文')) {
  ...
  const statCards = [
    { title: '作文项目学校', value: essayData.total, icon: <EditOutlined />, color: '#1677ff' },
  ...
  return (
    <div>
      ...
      <Card title={<Space><EditOutlined style={{ color: '#1677ff' }} /><span style={{ fontWeight: 600, color: '#1e293b' }}>作文项目学校明细</span></Space>}
