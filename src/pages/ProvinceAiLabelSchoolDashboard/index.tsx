...
export default function ProvinceAiLabelSchoolDashboard() {
  ...
  const handleExportCSV = () => {
    const headers = ['学校名称', '学段', '状态', '试用产品', '汇报产品', '合作产品', '关键人', '所属街道', '城市', '区县', 'AI标签校', '备注'];
    ...
    a.href = url;
    a.download = `全省AI标签校名单_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  const districtNames = [...new Set(aiLabelData.schools.map((s) => s.districtName))];
  const stageNames = [...new Set(aiLabelData.schools.map((s) => s.stage).filter(Boolean))] as string[];
  ...
  return (
    <div>
      ...
      <SchoolAnalytics schools={aiLabelData.schools} groupBy="city" groupLabel="城市" />
