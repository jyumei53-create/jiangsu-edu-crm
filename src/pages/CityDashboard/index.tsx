...
export default function CityDashboard() {
  ...
  const districtTable = city.districts.map((d) => {
    const s = {
      ...
      cooperating: d.schools.filter((x) => x.status === '已合作').length,
      trialing: d.schools.filter((x) => x.status === '试用中').length,
      reported: d.schools.filter((x) => x.status === '已汇报').length,
      pending: d.schools.filter((x) => x.status === '待开发').length,
    };
    return {
      name: d.name,
      ...s,
