...
export default function ProvincePrivateSchoolDashboard() {
  ...
  const handleExportCSV = () => {
    ...
    const rows = privateData.schools.map((s) => [
      ...
      (s as any).cityName || '', (s as any).districtName || '',
    ...
    a.href = url;
    a.download = `全省民办校名单_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  const columns = [
    ...
    { title: '所属城市', dataIndex: 'cityName', key: 'cityName', width: 100, render: (t: string) => <Tag>{t}</Tag> },
  ...
  return (
    <div>
      ...
      <SchoolAnalytics schools={privateData.schools} groupBy="city" groupLabel="城市" />

      <Card
        title={
          <Space>
            <BankOutlined style={{ color: '#1677ff' }} />
            <span style={{ fontWeight: 600, color: '#1e293b' }}>民办学校明细</span>
          </Space>
        }
      ...
      >
        <Table
          rowKey="id"
          columns={columns}
          dataSource={privateData.schools}
          size="middle"
          scroll={{ x: 1000 }}
